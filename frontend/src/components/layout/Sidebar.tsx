...
export const Sidebar = ({ isOpen, isCollapsed, onClose, onToggleCollapse }: SidebarProps) => {
  ...
  const navItems = admin ? [...USER_NAV, ...ADMIN_ONLY_NAV] : USER_NAV;
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center h-16 border-b border-slate-800 shrink-0 ${isCollapsed ? 'justify-center px-3' : 'px-5 gap-3'}`}>
        <div aria-hidden="true" className="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-600 shrink-0">
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <Icon aria-hidden="true" className={`shrink-0 h-[18px] w-[18px] ${isActive ? 'text-blue-400' : 'text-slate-400 group-hover:text-white'}`} />
      ...
      <div className="border-t border-slate-800 px-2 py-3 space-y-0.5 shrink-0">
        ...
        >
          {({ isActive }) => (
            <>
              {isActive && <span aria-hidden="true" className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-blue-400 rounded-r-full" />}
              ...
              {isCollapsed && (
                <span aria-hidden="true" className="pointer-events-none absolute left-full ml-2 px-2 py-1 text-xs font-medium text-white bg-slate-800 rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-50">Settings</span>
              )}
            </>
          )}
        </NavLink>
        ...
        <button
          ...
          className={`hidden lg:flex items-center rounded-lg transition-colors duration-100 text-slate-500 hover:bg-slate-800 hover:text-white ${isCollapsed ? 'justify-center mx-auto w-11 h-11' : 'gap-3 w-full px-3 py-2.5'}`}
        >
          {isCollapsed
            ? <ChevronRight aria-hidden="true" className="h-[18px] w-[18px]" />
            : <><ChevronLeft aria-hidden="true" className="h-[18px] w-[18px]" /><span className="text-sm font-medium">Collapse</span></>
          }
        </button>
        ...
        >
          <ChevronLeft aria-hidden="true" className="h-[18px] w-[18px]" /> Close menu
        </button>
      </div>
    </aside>
  );
