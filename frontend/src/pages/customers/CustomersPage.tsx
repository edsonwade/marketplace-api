...
const CustomerModal = ({ customer, onClose, onSave, isSaving }: ModalProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setError(null);
      await onSave(form, customer?.customerId);
    } catch {
          ...
          {!customers || customers.length === 0 ? (
            ...
            customers.map((c) => (
              <TableRow key={c.customerId}>
                <TableCell className="font-mono text-xs text-slate-400">#{c.customerId}</TableCell>
                <TableCell>
                  <div className="flex items-center gap-2.5">
                    <div
                      aria-hidden="true"
                      className="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 text-xs font-bold shrink-0"
                    >
                ...
                <TableCell className="text-slate-500 max-w-[200px] truncate">{c.address}</TableCell>
                <TableCell>
                  <div className="flex items-center justify-end gap-1.5">
                    {deleteId === c.customerId ? (
                      <>
                        <span className="text-xs text-slate-500 mr-1">Delete?</span>
                        <Button size="sm" variant="danger" onClick={() => handleDelete(c.customerId)} isLoading={deleteCustomer.isPending}>
                          Confirm
                        </Button>
