...
export const RegisterPage = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 py-8">
      <div className="w-full max-w-[420px]">
        {/* Card — identical structure to LoginPage */}
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-700">
          ...
          <div className="px-8 py-8">
            ...
            <div className="flex flex-col items-center mb-8">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Join the marketplace platform</p>
            </div>

            {registerMutation.isError && (
              <Alert variant="error" message={getRegisterError(registerMutation.error)} className="mb-5" />
            )}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
                ...
                {password.length > 0 && (
                  <div className="mt-2">
                    <div className="flex gap-1 mb-2">
                      {[1, 2, 3, 4, 5].map((n) => (
                        <div key={n} className={`h-1 flex-1 rounded-full transition-all duration-300 ${n <= strengthPassed ? strengthColor : 'bg-slate-200 dark:bg-slate-700'}`} />
                      ))}
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                      {RULES.map((rule) => {
                        ...
                        return (
                          <div key={rule.label} className={`flex items-center gap-1.5 text-[11px] ${ok ? 'text-emerald-500' : 'text-slate-400 dark:text-slate-500'}`}>
                            {ok
              ...
              <Button type="submit" className="w-full mt-2" size="lg"
                isLoading={registerMutation.isPending} disabled={registerMutation.isPending}>
